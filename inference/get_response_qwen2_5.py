...
class VideoProcessor:
    """Video processing main class - using local Qwen model"""
    ...
    def __init__(self, config):
        ...
        self.logger.info(f"Loading Qwen model: {self.model_path}")
        ...
        try:
            self.model = LLM(
                ...
                limit_mm_per_prompt={"image": 256, "video": 5},
                max_model_len=32768,
    ...
    def _append_result_to_file(self, result):
        ...
        with self._output_file_lock:
            try:
                ...
                # Backup handling also needs modification
                ...
                try:
                    ...
                    # Backup also only keeps required fields
                    clean_result = {
                        "index": result["index"],
                        "video1_path": result["video1_path"],
                        "video2_path": result["video2_path"],
    ...
    def process_all(self):
        ...
        self.logger.info("="*60)
        self.logger.info("Starting batch processing (using local Qwen model)")
        self.logger.info(f"Batch size: {self.batch_size}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Video comparison analysis processing program - using local Qwen model (Linux)',
...
def main():
    ...
    print("="*60)
    print("Video Comparison Analysis Processing Program (Linux)")
