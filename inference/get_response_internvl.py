...
# Fixed folder path configuration
MODEL_FOLDER = ""  # Model folder path
OUTPUT_FOLDER = "response"  # Output folder
LOG_FOLDER = "logs"  # Log folder


def load_video_frames(video_path: str, max_frames: int = 32, image_size: int = 448) -> List[Image.Image]:
    """
    Extract fixed number of frames (32 frames) from video and preprocess to reduce token count
    """
    frames = []
    ...
    try:
        ...
        if total_frames == 0:
            # If unable to get total frame count, iterate to count
            frame_count = 0
        ...
        for frame in container.decode(stream):
            if frame_idx in collected_indices:
                ...
                # Important: Resize image to reduce token count
                # This is equivalent to max_num=1 setting in official code
...
class VideoProcessor:
    """Video processing main class - using vLLM InternVL model"""
    ...
    def __init__(self, config):
        ...
        self.logger.info(f"Loading InternVL model: {self.model_path}")
        self.logger.info(f"GPU memory utilization setting: {self.gpu_memory_utilization}")
        self.logger.info(f"Thinking mode enabled: {self.thinking}")
        
        try:
            # InternVL vLLM configuration
            self.model = LLM(
                model=self.model_path,
                tensor_parallel_size=self.tensor_parallel_size,
                gpu_memory_utilization=self.gpu_memory_utilization,
                trust_remote_code=True,
                max_model_len=32768,  # InternVL typically supports longer context
                limit_mm_per_prompt={"image": 256},  # InternVL primarily processes images
                dtype="bfloat16"
            )
            self.logger.info("✅ InternVL model loaded successfully")
        except Exception as e:
            self.logger.error(f"❌ Model loading failed: {e}")
            raise
        
        # Set different sampling parameters based on whether thinking mode is enabled
        if self.thinking:
        ...
        # If thinking mode is enabled, add thinking prompt
        if self.thinking:
...
"""
    ...
    def process_video_pairs_batch(self, entries):
        ...
        for entry in entries:
            ...
            try:
                # Load videos directly without path conversion
                frames1 = load_video_frames(video1_path, self.max_frames_per_video)
                frames2 = load_video_frames(video2_path, self.max_frames_per_video)
                
                # Log file sizes (if files exist)
                try:
                    if os.path.exists(video1_path):
                        size1_mb = os.path.getsize(video1_path) / (1024 * 1024)
                        size2_mb = os.path.getsize(video2_path) / (1024 * 1024) if os.path.exists(video2_path) else 0
                        self.logger.info(f"Video1 size: {size1_mb:.2f}MB, Video2 size: {size2_mb:.2f}MB")
                except:
                    pass
                
            except Exception as e:
                self.logger.error(f"Unable to load videos: {str(e)}")
                raise
            ...
            # Build prompt text
            # InternVL uses <image> token to indicate image position
            image_tokens_1 = ''.join(['<image>' for _ in range(len(frames1))])
            ...
            batch_images.append(all_frames)
        ...
        for prompt, images in zip(batch_prompts, batch_images):
            llm_input = {
                "prompt": prompt,
                "multi_modal_data": {
                    "image": images  # InternVL accepts image list
                }
            }
        ...
        for output in outputs:
            ...
            # If thinking mode is enabled, extract final answer
    ...
    def process_all(self):
        ...
        self.logger.info("="*60)
        self.logger.info("Starting batch processing (using vLLM InternVL model)")
        self.logger.info(f"Batch size: {self.batch_size}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='InternVL video comparison analysis processing program - using vLLM',
...
def main():
    ...
    print("="*60)
    print("InternVL Video Comparison Analysis Processing Program (vLLM)")
